import{r as q,l as T,d as u,b as f,m as M,c as V,n as P,p as D,q as U}from"./workbench-DEQ833e2.js";const r={captureForm:document.querySelector("#captureForm"),checkInVehicleList:document.querySelector("#checkInVehicleList"),selectedVehicleSummary:document.querySelector("#selectedVehicleSummary"),accessoryList:document.querySelector("#accessoryList"),customServiceInput:document.querySelector("#customServiceInput"),addServiceItemBtn:document.querySelector("#addServiceItemBtn"),addAccessoryBtn:document.querySelector("#addAccessoryBtn"),saveSetBtn:document.querySelector("#saveSetBtn"),captureStatus:document.querySelector("#captureStatus"),cameraOverlay:document.querySelector("#cameraOverlay"),cameraTitle:document.querySelector("#cameraTitle"),cameraVideo:document.querySelector("#cameraVideo"),cameraCanvas:document.querySelector("#cameraCanvas"),closeCameraBtn:document.querySelector("#closeCameraBtn"),cancelCameraBtn:document.querySelector("#cancelCameraBtn"),shutterCameraBtn:document.querySelector("#shutterCameraBtn")},a={serviceItems:[],accessoryEntries:[],checkInSets:[],selectedCaptureSetId:"",vehicleThumbUrls:new Map,cameraTarget:null,cameraStream:null};function S(){return{id:`entry-${Date.now()}-${Math.floor(Math.random()*1e4)}`,itemIds:[],notes:"",photos:[]}}function w(e){return(e||"").trim().replace(/\s+/g," ")}function x(e,t){return[...e.filter(c=>c.id!==t.id),t].sort((c,s)=>c.sortOrder!==s.sortOrder?c.sortOrder-s.sortOrder:c.name.localeCompare(s.name,"zh-Hant"))}function n(e,t){r.captureStatus.textContent=e,r.captureStatus.className="status-text",t&&r.captureStatus.classList.add(`is-${t}`)}function y(){return!!(window.matchMedia?.("(pointer: coarse)").matches&&navigator.mediaDevices?.getUserMedia)}function O(){return y()?"拍安裝 / 維修 / 保養相片":"拍安裝 / 維修 / 保養相片或上傳"}function j(e=""){return y()?`
  ...
  `:""}function F(e){return!e?.width||!e?.height?"4 / 3":`${e.width} / ${e.height}`}function H(e,t,c,s){return`
    <figure class="upload-cover">
      <div class="upload-cover-frame" style="aspect-ratio: ${F(e)};">
  ...
  `}function N(){r.addAccessoryBtn.classList.remove("is-flashing"),window.requestAnimationFrame(()=>{r.addAccessoryBtn.classList.add("is-flashing"),window.setTimeout(()=>{r.addAccessoryBtn.classList.remove("is-flashing")},1200)})}function z(){return a.checkInSets.find(e=>e.id===a.selectedCaptureSetId)||null}function g(){return a.accessoryEntries.some(e=>e.itemIds.length>0||e.photos.length>0||w(e.notes))}function C(){a.accessoryEntries.forEach(e=>{e.photos.forEach(t=>f(t))})}function I(){C(),a.accessoryEntries=[S()],d(),m()}function h(){const e=z();if(!e){r.selectedVehicleSummary.innerHTML=`
  ...
  `).join(""),r.accessoryList.querySelectorAll("[data-remove-entry]").forEach(e=>{e.addEventListener("click",()=>{const t=a.accessoryEntries.findIndex(c=>c.id===e.dataset.removeEntry);t!==-1&&(a.accessoryEntries[t].photos.forEach(c=>f(c)),a.accessoryEntries.splice(t,1),d())})}),r.accessoryList.querySelectorAll("[data-entry-item]").forEach(e=>{e.addEventListener("click",()=>{const[t,c]=e.dataset.entryItem.split(":"),s=a.accessoryEntries.find(i=>i.id===t);if(s){if(s.itemIds.includes(c))s.itemIds=s.itemIds.filter(i=>i!==c);else{const i=new Set(s.itemIds);i.add(c),s.itemIds=a.serviceItems.map(o=>o.id).filter(o=>i.has(o))}d()}})}),r.accessoryList.querySelectorAll("[data-remove-accessory-photo]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation();const[c,s]=e.dataset.removeAccessoryPhoto.split(":"),i=a.accessoryEntries.find(l=>l.id===c);if(!i)return;const o=i.photos.findIndex(l=>l.localId===s);o!==-1&&(f(i.photos[o]),i.photos.splice(o,1),d())})}),r.accessoryList.querySelectorAll("[data-camera-entry]").forEach(e=>{e.addEventListener("click",async t=>{if(!y()||t.target.closest("button"))return;const c=a.accessoryEntries.find(s=>s.id===e.dataset.cameraEntry);!c||c.photos.length>0||(t.preventDefault(),await E({kind:"accessory",entryId:c.id}))})}),r.accessoryList.querySelectorAll("[data-open-entry-camera]").forEach(e=>{e.addEventListener("click",async t=>{t.preventDefault(),t.stopPropagation(),await E({kind:"accessory",entryId:e.dataset.openEntryCamera})})}),r.accessoryList.querySelectorAll("[data-open-entry-library]").forEach(e=>{e.addEventListener("click",t=>{t.preventDefault(),t.stopPropagation(),r.accessoryList.querySelector(`#library-${e.dataset.openEntryLibrary}`)?.click()})}),r.accessoryList.querySelectorAll('input[type="file"]').forEach(e=>{e.addEventListener("change",async()=>{const t=e.id.replace("upload-","").replace("library-",""),c=a.accessoryEntries.find(s=>s.id===t);if(!(!c||!e.files?.length))try{await k(c,e.files[0])}catch(s){n(u(s),"danger")}finally{e.value=""}})}),r.accessoryList.querySelectorAll("textarea").forEach(e=>{e.addEventListener("input",()=>{const t=a.accessoryEntries.find(c=>c.id===e.id.replace("notes-",""));t&&(t.notes=e.value)})}),m()}async function k(e,t){if(t){if(e.photos.length>0){n("每個項目只可上傳一張相片。要再加入相片，請按「加入更多項目」。","danger"),N();return}n("處理項目相片中...",""),e.photos=[await V(t)],d(),n("已加入項目相片。","success")}}async function J(){try{return await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:"environment"}},audio:!1})}catch{return navigator.mediaDevices.getUserMedia({video:!0,audio:!1})}}async function B(){a.cameraStream&&(a.cameraStream.getTracks().forEach(e=>e.stop()),a.cameraStream=null),r.cameraVideo.srcObject=null}async function p(){r.cameraOverlay.hidden=!0,document.body.classList.remove("camera-open"),a.cameraTarget=null,await B()}async function E(e){if(y()){r.cameraTitle.textContent="拍安裝 / 維修 / 保養相片",a.cameraTarget=e,r.cameraOverlay.hidden=!1,document.body.classList.add("camera-open");try{a.cameraStream=await J(),r.cameraVideo.srcObject=a.cameraStream,await r.cameraVideo.play()}catch(t){await p(),n(u(t),"danger")}}}function K(e){const t=Date.now();return typeof File=="function"?new File([e],`service-${t}.jpg`,{type:"image/jpeg",lastModified:t}):(e.name=`service-${t}.jpg`,e)}async function Q(){const e=a.cameraTarget;if(!e)return;const t=r.cameraVideo.videoWidth,c=r.cameraVideo.videoHeight;if(!t||!c){n("未能取得相機影像，請再試一次。","danger");return}r.shutterCameraBtn.disabled=!0;try{r.cameraCanvas.width=t,r.cameraCanvas.height=c;const s=r.cameraCanvas.getContext("2d");if(!s)throw new Error("無法建立拍照畫布。");s.drawImage(r.cameraVideo,0,0,t,c);const i=await new Promise((v,A)=>{r.cameraCanvas.toBlob(b=>b?v(b):A(new Error("拍照失敗。")),"image/jpeg",.92)}),o=K(i);await p();const l=a.accessoryEntries.find(v=>v.id===e.entryId);if(!l){n("找不到目前項目，請重新拍照。","danger");return}await k(l,o)}catch(s){n(u(s),"danger")}finally{r.shutterCameraBtn.disabled=!1}}function X(){return a.selectedCaptureSetId?a.accessoryEntries.length?a.accessoryEntries.find(t=>!t.itemIds.length||!t.photos.length)?"每個項目都需要至少選 1 項分類並上傳 1 張相片。":"":"請至少加入 1 個安裝、維修或保養項目。":"請先選擇已 Check-in 車輛。"}function Y(){return a.accessoryEntries.map(e=>({itemIds:[...e.itemIds],notes:e.notes||"",photos:e.photos}))}async function $(){const e=w(r.customServiceInput.value);if(!e){n("請先輸入未有的項目名稱。","danger"),r.customServiceInput.focus();return}r.addServiceItemBtn.disabled=!0;try{const t=await D(e);a.serviceItems=x(a.serviceItems,t),r.customServiceInput.value="";const c=a.accessoryEntries[a.accessoryEntries.length-1];c&&!c.itemIds.includes(t.id)&&(c.itemIds=[...c.itemIds,t.id]),d(),n(`已加入新項目「${t.name}」，之後所有用戶都可直接選用。`,"success")}catch(t){n(u(t),"danger")}finally{r.addServiceItemBtn.disabled=!1}}async function Z(e){e.preventDefault();const t=X();if(t){n(t,"danger");return}r.saveSetBtn.disabled=!0,n("正在上傳安裝維修保養資料...","");try{const c=await U(a.selectedCaptureSetId,Y());I(),await L({keepSelection:!0}),n(`已為案件 ${c.reference} 新增安裝維修保養資料。`,"success")}catch(c){n(u(c),"danger")}finally{r.saveSetBtn.disabled=!1}}function ee(){r.addAccessoryBtn.addEventListener("click",()=>{a.accessoryEntries.push(S()),d(),n("已新增一個服務項目。",""),window.requestAnimationFrame(()=>{r.accessoryList.lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})})}),r.addServiceItemBtn.addEventListener("click",$),r.customServiceInput.addEventListener("keydown",async e=>{e.key==="Enter"&&(e.preventDefault(),await $())}),r.captureForm.addEventListener("submit",Z),r.closeCameraBtn.addEventListener("click",p),r.cancelCameraBtn.addEventListener("click",p),r.shutterCameraBtn.addEventListener("click",Q),r.cameraOverlay.addEventListener("click",async e=>{e.target===r.cameraOverlay&&await p()}),window.addEventListener("beforeunload",()=>{B(),C()})}async function te(){await q("../index.html"),a.accessoryEntries=[S()],d(),n("正在載入已 Check-in 車輛與服務項目...","");try{const[e]=await Promise.all([T(),L()]);a.serviceItems=e,d(),n(a.checkInSets.length?"已連接 Supabase，可選擇車輛後開始輸入安裝維修保養資料。":"已連接 Supabase，但暫時未有已 Check-in 車輛。",a.checkInSets.length?"success":"")}catch(e){n(u(e),"danger")}ee()}te();
