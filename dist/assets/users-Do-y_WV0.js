import{r as h,d as u,E as m,F as U}from"./workbench-DXxRXhVJ.js";const w="20260318-3",s={currentUserEmail:document.querySelector("#currentUserEmail"),usersStatus:document.querySelector("#usersStatus"),usersList:document.querySelector("#usersList"),refreshUsersBtn:document.querySelector("#refreshUsersBtn"),createUserForm:document.querySelector("#createUserForm"),newUserEmail:document.querySelector("#newUserEmail"),newUserPassword:document.querySelector("#newUserPassword"),newUserGroup:document.querySelector("#newUserGroup"),createUserBtn:document.querySelector("#createUserBtn")},c={users:[],currentUserId:""};function y(e){s.currentUserEmail&&(s.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-"),c.currentUserId=e?.id||""}function a(e,r){s.usersStatus.textContent=e,s.usersStatus.className="status-text",r&&s.usersStatus.classList.add(`is-${r}`)}async function p(e,r={}){if(!m)throw new Error("Supabase 未設定。");let{data:t,error:n}=await m.auth.getSession();if(n)throw n;if(t.session?.refresh_token){const f=await m.auth.refreshSession({refresh_token:t.session.refresh_token});if(f.error)throw f.error;t=f.data}if(!t.session?.access_token)throw new Error("目前登入狀態無效，請先重新登入。");const i=await fetch(`${U.supabaseUrl}/functions/v1/user-admin`,{method:"POST",headers:{"Content-Type":"application/json",apikey:U.supabasePublishableKey,Authorization:`Bearer ${t.session.access_token}`},body:JSON.stringify({action:e,...r})});let o=null,l="";try{l=await i.text(),o=l?JSON.parse(l):null}catch{o=null}if(!i.ok)throw new Error(o?.error||o?.message||l||`用戶管理操作失敗（HTTP ${i.status}）。`);if(!o?.ok)throw new Error(o?.error||"用戶管理操作失敗。");return o}function g(){if(!c.users.length){s.usersList.innerHTML=`
    ...
    `}).join(""),s.usersList.querySelectorAll("[data-save-user]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.dataset.saveUser,n=s.usersList.querySelector(`[data-user-group="${r}"]`)?.value||"";if(!n){a("請先選擇群組。","danger");return}e.disabled=!0;try{await p("updateGroup",{userId:r,group:n}),await d(),a("用戶權限已更新。","success")}catch(i){a(u(i),"danger")}finally{e.disabled=!1}})}),s.usersList.querySelectorAll("[data-delete-user]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.dataset.deleteUser,t=c.users.find(n=>n.id===r);if(t&&window.confirm(`確定刪除用戶 ${t.email||t.id}？此操作不可還原。`)){e.disabled=!0;try{await p("deleteUser",{userId:r}),await d(),a("用戶已刪除。","success")}catch(n){a(u(n),"danger")}finally{e.disabled=!1}}})})}async function d(){const e=await p("listUsers");c.users=e.users||[],g()}async function S(e){e.preventDefault();const r=s.newUserEmail.value.trim(),t=s.newUserPassword.value,n=s.newUserGroup.value;if(!r||!t||!n){a("請完整輸入 Email、Password 和群組。","danger");return}s.createUserBtn.disabled=!0;try{await p("createUser",{email:r,password:t,group:n}),s.newUserEmail.value="",s.newUserPassword.value="",s.newUserGroup.value="staff",await d(),a("新用戶已建立。","success")}catch(i){a(u(i),"danger")}finally{s.createUserBtn.disabled=!1}}function v(){s.refreshUsersBtn.addEventListener("click",async()=>{s.refreshUsersBtn.disabled=!0;try{await d(),a("已重新載入用戶資料。","success")}catch(e){a(u(e),"danger")}finally{s.refreshUsersBtn.disabled=!1}}),s.createUserForm.addEventListener("submit",S)}async function E(){const e=await h(["superadmin","supreadmin"],"../index.html");y(e),a(`正在載入用戶資料...（client ${w}）`,""),v();try{await d(),a(`可管理所有用戶的群組、新增與刪除。（client ${w}）`,"success")}catch(r){a(`[client ${w}] ${u(r)}`,"danger")}}E();
