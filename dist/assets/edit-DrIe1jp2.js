import{r as F,d,I as b,J as C,K as j,y as q,L as U,M as D,N as y,O as $,p as v,Q as B,B as x,C as I,R as T,q as E,P,S as N,T as k,U as A}from"./workbench-4gzxfYMo.js";const H=new URLSearchParams(window.location.search).get("date"),a={selectedDate:H||null,currentMonth:null,selectedFilterId:"",filters:[],captureSets:[],uploadMap:{}},n={currentUserEmail:document.querySelector("#currentUserEmail"),monthLabel:document.querySelector("#monthLabel"),calendarGrid:document.querySelector("#calendarGrid"),selectedDateTitle:document.querySelector("#selectedDateTitle"),selectedDateMeta:document.querySelector("#selectedDateMeta"),setList:document.querySelector("#setList"),filterList:document.querySelector("#filterList"),applyDayFilterBtn:document.querySelector("#applyDayFilterBtn"),downloadDayBtn:document.querySelector("#downloadDayBtn"),selectedFilterChip:document.querySelector("#selectedFilterChip"),editStatus:document.querySelector("#editStatus"),prevMonthBtn:document.querySelector("#prevMonthBtn"),nextMonthBtn:document.querySelector("#nextMonthBtn")};function R(e){n.currentUserEmail&&(n.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-")}function c(e,t){n.editStatus.textContent=e,n.editStatus.className="status-text",t&&n.editStatus.classList.add(`is-${t}`)}function Y(e){const t=e.length,s=e.reduce((r,i)=>r+T(i),0);return`${t} 組案件，${s} 張相片`}async function O(e,t){const s=await v(e.storagePath,{width:720,height:540});return N(e.adjustments)?k(s,e.adjustments,{width:720,height:540,mimeType:"image/jpeg",quality:.86}):s}function w(){n.monthLabel.textContent=x(a.currentMonth);const e=I(a.currentMonth.getFullYear(),a.currentMonth.getMonth());n.calendarGrid.innerHTML=e.map(t=>{if(!t)return'<div class="calendar-day is-empty"></div>';const s=`${t.getFullYear()}-${`${t.getMonth()+1}`.padStart(2,"0")}-${`${t.getDate()}`.padStart(2,"0")}`,r=a.uploadMap[s]||0;return`
  ...
  `}async function z(){const e=[...n.setList.querySelectorAll("[data-photo-id]")];await Promise.all(e.map(async t=>{const s=t.dataset.photoId,r=t.querySelector("img"),i=a.captureSets.flatMap(o=>y(o)).find(o=>o.photo.id===s)?.photo;if(!(!i||!r))try{r.src=await O(i,"thumb")}catch(o){if(E(o)){r.src=P;return}t.querySelector(".photo-body p").textContent=d(o)}}))}async function g(){n.selectedDateTitle.textContent=q(a.selectedDate);try{a.captureSets=await U(a.selectedDate)}catch(e){a.captureSets=[],n.setList.innerHTML=`
      <div class="empty-state">
        <strong>讀取失敗</strong>
        <p class="muted-copy">${d(e)}</p>
      </div>
    `,c(d(e),"danger");return}if(n.selectedDateMeta.textContent=Y(a.captureSets),!a.captureSets.length){n.setList.innerHTML=`
      <div class="empty-state">
        <strong>這一天沒有相片</strong>
        <p class="muted-copy">回到 Check-in 或安裝維修保養頁面建立新案件，日曆便會自動出現標記。</p>
      </div>
    `;return}n.setList.innerHTML=a.captureSets.map(e=>{const t=D(e),r=y(e).map(({photo:i,kindLabel:o,itemName:u,itemNames:p})=>{const h=a.filters.find(l=>l.id===i.savedFilterId)?.name||(i.savedFilterId?"已套用 filter":"未套用 filter"),m=(p||[]).map(l=>`<span class="meta-chip">${l}</span>`).join("");return`
        <article class="photo-card" data-photo-id="${i.id}">
          <img alt="${i.fileName}">
          <div class="photo-body">
            <div class="chip-row">
          ...
          <div class="photo-actions">
            <a class="secondary-button" href="./detail.html?photo=${encodeURIComponent(i.id)}">進階調色</a>
          </div>
        </article>
      `}).join("")||G(e);return`
