import{r as L,d,A as b,B as j,C as q,v as B,D as C,E as D,F as y,G as $,n as w,H as x,y as U,z as I,I as T,J as E,K as N,L as k}from"./workbench-ChSf54Qe.js";const A=new URLSearchParams(window.location.search).get("date"),a={selectedDate:A||null,currentMonth:null,selectedFilterId:"",filters:[],captureSets:[],uploadMap:{}},n={currentUserEmail:document.querySelector("#currentUserEmail"),monthLabel:document.querySelector("#monthLabel"),calendarGrid:document.querySelector("#calendarGrid"),selectedDateTitle:document.querySelector("#selectedDateTitle"),selectedDateMeta:document.querySelector("#selectedDateMeta"),setList:document.querySelector("#setList"),filterList:document.querySelector("#filterList"),applyDayFilterBtn:document.querySelector("#applyDayFilterBtn"),downloadDayBtn:document.querySelector("#downloadDayBtn"),selectedFilterChip:document.querySelector("#selectedFilterChip"),editStatus:document.querySelector("#editStatus"),prevMonthBtn:document.querySelector("#prevMonthBtn"),nextMonthBtn:document.querySelector("#nextMonthBtn")};function H(e){n.currentUserEmail&&(n.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-")}function c(e,t){n.editStatus.textContent=e,n.editStatus.className="status-text",t&&n.editStatus.classList.add(`is-${t}`)}function P(e){const t=e.length,r=e.reduce((s,o)=>s+T(o),0);return`${t} 組案件，${r} 張相片`}async function R(e,t){const r=await w(e.storagePath,{width:720,height:540});return E(e.adjustments)?N(r,e.adjustments,{width:720,height:540,mimeType:"image/jpeg",quality:.86}):r}function M(){n.monthLabel.textContent=U(a.currentMonth);const e=I(a.currentMonth.getFullYear(),a.currentMonth.getMonth());n.calendarGrid.innerHTML=e.map(t=>{if(!t)return'<div class="calendar-day is-empty"></div>';const r=`${t.getFullYear()}-${`${t.getMonth()+1}`.padStart(2,"0")}-${`${t.getDate()}`.padStart(2,"0")}`,s=a.uploadMap[r]||0;return`
  ...
  `).join(""),n.filterList.querySelectorAll("[data-filter-id]").forEach(t=>{t.addEventListener("click",()=>{a.selectedFilterId=t.dataset.filterId,v()})})}async function Y(){const e=[...n.setList.querySelectorAll("[data-photo-id]")];await Promise.all(e.map(async t=>{const r=t.dataset.photoId,s=t.querySelector("img"),o=a.captureSets.flatMap(i=>y(i)).find(i=>i.photo.id===r)?.photo;if(!(!o||!s))try{s.src=await R(o,"thumb")}catch(i){t.querySelector(".photo-body p").textContent=d(i)}}))}async function f(){n.selectedDateTitle.textContent=B(a.selectedDate);try{a.captureSets=await C(a.selectedDate)}catch(e){a.captureSets=[],n.setList.innerHTML=`
      <div class="empty-state">
        <strong>讀取失敗</strong>
        <p class="muted-copy">${d(e)}</p>
      </div>
    `,c(d(e),"danger");return}if(n.selectedDateMeta.textContent=P(a.captureSets),!a.captureSets.length){n.setList.innerHTML=`
      <div class="empty-state">
    ...
    `}).join(""),await Y()}async function G(){a.filters=b(await j()),v()}async function z(){if(!a.selectedFilterId){c("請先選擇一個已儲存 filter。","danger");return}n.applyDayFilterBtn.disabled=!0,c("正在為當天相片套用 filter...","");try{const e=await k(a.selectedDate,a.selectedFilterId);c(`已為 ${e} 張相片套用 filter。`,"success"),await f()}catch(e){c(d(e),"danger")}finally{n.applyDayFilterBtn.disabled=!1}}function J(e,t){const r=URL.createObjectURL(e),s=document.createElement("a");s.href=r,s.download=t,s.click(),setTimeout(()=>URL.revokeObjectURL(r),1e3)}async function O(){if(!a.captureSets.length){c("這一天沒有可下載的相片。","danger");return}n.downloadDayBtn.disabled=!0,c("正在打包當天相片...","");try{const e=new window.JSZip;for(const r of a.captureSets){const s=e.folder($(D(r)));let o=1;for(const{photo:i,itemName:u,itemNames:p}of y(r)){const h=await w(i.storagePath),l=await x(h,i.adjustments,{mimeType:i.mimeType==="image/png"?"image/png":"image/jpeg",quality:.92}),g=i.mimeType==="image/png"?"png":"jpg",S=(p||[]).join("+")||u||i.itemId,F=i.kind==="vehicle"?`vehicle-${o}.${g}`:`accessory-${$(S||`item-${o}`)}-${o}.${g}`;s.file(F,l),o+=1}}const t=await e.generateAsync({type:"blob"});J(t,`garage-photos-${a.selectedDate}.zip`),c("下載壓縮包已開始。","success")}catch(e){c(d(e),"danger")}finally{n.downloadDayBtn.disabled=!1}}async function m(e){a.currentMonth=new Date(e.getFullYear(),e.getMonth(),1);try{a.uploadMap=await q(a.currentMonth.getFullYear(),a.currentMonth.getMonth()),M()}catch(t){c(d(t),"danger")}}function K(){const e=new URL(window.location.href);e.searchParams.set("date",a.selectedDate),window.history.replaceState({},"",e)}function Q(){n.applyDayFilterBtn.addEventListener("click",z),n.downloadDayBtn.addEventListener("click",O),n.prevMonthBtn.addEventListener("click",async()=>{const e=new Date(a.currentMonth.getFullYear(),a.currentMonth.getMonth()-1,1);await m(e)}),n.nextMonthBtn.addEventListener("click",async()=>{const e=new Date(a.currentMonth.getFullYear(),a.currentMonth.getMonth()+1,1);await m(e)})}async function W(){const e=await L("../index.html");H(e);const t=a.selectedDate?new Date(`${a.selectedDate}T00:00:00`):new Date;a.selectedDate||(a.selectedDate=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,"0")}-${String(t.getDate()).padStart(2,"0")}`),Q(),c("讀取日曆與 filter 中...","");try{await G(),await m(t),await f(),c("已連接 Supabase。","success")}catch(r){c(d(r),"danger")}}W();
