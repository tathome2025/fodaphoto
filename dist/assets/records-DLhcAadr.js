    ...
    `,s("這一天暫時未有車輛處理紀錄。","");return}n.recordList.innerHTML=t.captureSets.map(e=>{const a=e.activityOnDate.hasCheckIn?`
        <section class="activity-block">
          ...
          <div class="record-photo-grid">
            ${(e.vehiclePhotos||[]).map(r=>p(r,"車輛照")).join("")}
          </div>
        </section>
      `:"",c=e.activityOnDate.serviceEntries.map((r,u)=>{const i=r.photos[0];return`
        <section class="activity-block">
          <div class="activity-head">
            <div>
          ...
          <div class="record-photo-grid">
            ${(r.photos||[]).map(d=>p(d,r.itemLabel||"工序照片")).join("")}
          </div>
        </section>
      `}).join("");return`
