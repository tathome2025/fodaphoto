import{r as d,d as r,n as u,p as k,P as m,q as v,w as C}from"./workbench-VwRDttHn.js";const s={currentUserEmail:document.querySelector("#currentUserEmail"),checkoutVehicleList:document.querySelector("#checkoutVehicleList"),checkoutBtn:document.querySelector("#checkoutBtn"),checkoutStatus:document.querySelector("#checkoutStatus")},c={checkInSets:[],selectedCaptureSetId:"",vehicleThumbUrls:new Map};function S(e){s.currentUserEmail&&(s.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-")}function i(e,t){s.checkoutStatus.textContent=e,s.checkoutStatus.className="status-text",t&&s.checkoutStatus.classList.add(`is-${t}`)}function h(){return c.checkInSets.find(e=>e.id===c.selectedCaptureSetId)||null}function a(){s.checkoutBtn.disabled=!h()}function l(){if(!c.checkInSets.length){s.checkoutVehicleList.innerHTML=`
    ...
    `}).join(""),s.checkoutVehicleList.querySelectorAll("[data-select-capture-set]").forEach(e=>{e.addEventListener("click",()=>{c.selectedCaptureSetId=e.dataset.selectCaptureSet,l(),i("已選擇車輛，可按下 Check out。","")})}),a()}async function b(){const e=c.checkInSets.filter(t=>t.vehiclePhotos[0]?.storagePath&&!c.vehicleThumbUrls.has(t.id));e.length&&(await Promise.all(e.map(async t=>{try{const n=await k(t.vehiclePhotos[0].storagePath,{width:360,height:270});c.vehicleThumbUrls.set(t.id,n)}catch(n){c.vehicleThumbUrls.set(t.id,v(n)?m:"")}})),l())}async function o(){c.checkInSets=await u(48),c.checkInSets.some(e=>e.id===c.selectedCaptureSetId)||(c.selectedCaptureSetId=""),l(),await b()}async function f(){const e=h();if(!e){i("請先選擇一台要 Check out 的車輛。","danger");return}s.checkoutBtn.disabled=!0,i("正在完成 Check out...","");try{const t=await C(e.id);c.selectedCaptureSetId="",await o(),i(`案件 ${t.reference||e.id} 已 Check out，之後不再出現在安裝維修保養及 Check out 頁。`,"success"),document.querySelector(".capture-card")?.scrollIntoView({behavior:"smooth",block:"start"})}catch(t){i(r(t),"danger")}finally{a()}}function g(){s.checkoutBtn.addEventListener("click",f)}async function p(){const e=await d(["admin","superadmin","supreadmin"],"../index.html");S(e),i("正在載入已 Check-in 車輛...","");try{await o(),i(c.checkInSets.length?"已連接 Supabase，請選擇一台車並執行 Check out。":"已連接 Supabase，但暫時未有可 Check out 車輛。",c.checkInSets.length?"success":"")}catch(t){i(r(t),"danger")}g()}p();
