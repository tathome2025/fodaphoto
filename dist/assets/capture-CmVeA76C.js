import{r as M,m as T,d as u,c as p,n as P,e as U,p as V,P as D,q as O,u as x,v as j}from"./workbench-VwRDttHn.js";const r={captureForm:document.querySelector("#captureForm"),checkInVehicleList:document.querySelector("#checkInVehicleList"),accessoryList:document.querySelector("#accessoryList"),customServiceInput:document.querySelector("#customServiceInput"),addServiceItemBtn:document.querySelector("#addServiceItemBtn"),addAccessoryBtn:document.querySelector("#addAccessoryBtn"),saveSetBtn:document.querySelector("#saveSetBtn"),captureStatus:document.querySelector("#captureStatus"),cameraOverlay:document.querySelector("#cameraOverlay"),cameraTitle:document.querySelector("#cameraTitle"),cameraVideo:document.querySelector("#cameraVideo"),cameraCanvas:document.querySelector("#cameraCanvas"),closeCameraBtn:document.querySelector("#closeCameraBtn"),cancelCameraBtn:document.querySelector("#cancelCameraBtn"),shutterCameraBtn:document.querySelector("#shutterCameraBtn"),currentUserEmail:document.querySelector("#currentUserEmail")},a={serviceItems:[],accessoryEntries:[],checkInSets:[],selectedCaptureSetId:"",vehicleThumbUrls:new Map,cameraTarget:null,cameraStream:null};function v(){return{id:`entry-${Date.now()}-${Math.floor(Math.random()*1e4)}`,itemIds:[],photos:[]}}function F(e){return(e||"").trim().replace(/\s+/g," ")}function H(e,t){return[...e.filter(s=>s.id!==t.id),t].sort((s,c)=>s.sortOrder!==c.sortOrder?s.sortOrder-c.sortOrder:s.name.localeCompare(c.name,"zh-Hant"))}function n(e,t){r.captureStatus.textContent=e,r.captureStatus.className="status-text",t&&r.captureStatus.classList.add(`is-${t}`)}function N(e){r.currentUserEmail&&(r.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-")}function h(){return!!(window.matchMedia?.("(pointer: coarse)").matches&&navigator.mediaDevices?.getUserMedia)}function z(){return h()?"拍安裝 / 維修 / 保養相片":"拍安裝 / 維修 / 保養相片或上傳"}function R(e=""){return h()?`
    <div class="upload-action-row">
      <button class="primary-button upload-action-btn" type="button" data-open-entry-camera="${e}">拍照（開啟相機）</button>
      <button class="secondary-button upload-action-btn" type="button" data-open-entry-library="${e}">上傳相片（從相簿）</button>
    </div>
  `:""}function _(e){return!e?.width||!e?.height?"1 / 1":`${e.width} / ${e.height}`}function G(e,t,s,c){return`
  ...
  `}function W(){r.addAccessoryBtn.classList.remove("is-flashing"),window.requestAnimationFrame(()=>{r.addAccessoryBtn.classList.add("is-flashing"),window.setTimeout(()=>{r.addAccessoryBtn.classList.remove("is-flashing")},1200)})}function I(){return a.accessoryEntries.some(e=>e.itemIds.length>0||e.photos.length>0)}function C(){a.accessoryEntries.forEach(e=>{e.photos.forEach(t=>p(t))})}function w(){C(),a.accessoryEntries=[v()],d(),y()}function y(){if(!a.checkInSets.length){r.checkInVehicleList.innerHTML=`
    ...
    `}).join(""),r.checkInVehicleList.querySelectorAll("[data-select-capture-set]").forEach(e=>{e.addEventListener("click",()=>{const t=e.dataset.selectCaptureSet;t!==a.selectedCaptureSetId&&(I()&&(w(),n("已切換車輛，未儲存的服務草稿已清空。","danger")),a.selectedCaptureSetId=t,y())})})}async function X(){const e=a.checkInSets.filter(t=>t.vehiclePhotos[0]?.storagePath&&!a.vehicleThumbUrls.has(t.id));e.length&&(await Promise.all(e.map(async t=>{try{const s=await V(t.vehiclePhotos[0].storagePath,{width:360,height:270});a.vehicleThumbUrls.set(t.id,s)}catch(s){a.vehicleThumbUrls.set(t.id,O(s)?D:"")}})),y())}async function L(e={}){const t=e.keepSelection?a.selectedCaptureSetId:"";a.checkInSets=await P(24),t&&a.checkInSets.some(s=>s.id===t)?a.selectedCaptureSetId=t:e.autoSelectFirst===!1?a.selectedCaptureSetId="":a.selectedCaptureSetId=a.checkInSets[0]?.id||"",y(),await X()}function Y(e){return e.photos.length?e.photos.map(t=>G(t,t.fileName,"data-remove-accessory-photo",`${e.id}:${t.localId}`)).join(""):`
      <div class="upload-prompt">
        <strong>${z()}</strong>
        <small>每個項目只可上傳一張相片。</small>
        ${R(e.id)}
      </div>
    `}function J(e){return e.itemIds.length?`已選：${a.serviceItems.filter(t=>e.itemIds.includes(t.id)).map(t=>t.name).join(" + ")}`:"可選多於一項安裝、維修或保養項目。"}function K(e){return a.serviceItems.map(t=>`
