import{r as m,d as u,E as p,F as f}from"./workbench-DXxRXhVJ.js";const s={currentUserEmail:document.querySelector("#currentUserEmail"),usersStatus:document.querySelector("#usersStatus"),usersList:document.querySelector("#usersList"),refreshUsersBtn:document.querySelector("#refreshUsersBtn"),createUserForm:document.querySelector("#createUserForm"),newUserEmail:document.querySelector("#newUserEmail"),newUserPassword:document.querySelector("#newUserPassword"),newUserGroup:document.querySelector("#newUserGroup"),createUserBtn:document.querySelector("#createUserBtn")},c={users:[],currentUserId:""};function w(e){s.currentUserEmail&&(s.currentUserEmail.textContent=e?.email||e?.phone||e?.id||"-"),c.currentUserId=e?.id||""}function t(e,r){s.usersStatus.textContent=e,s.usersStatus.className="status-text",r&&s.usersStatus.classList.add(`is-${r}`)}async function l(e,r={}){if(!p)throw new Error("Supabase 未設定。");const{data:n,error:a}=await p.auth.getSession();if(a)throw a;if(!n.session?.access_token)throw new Error("目前登入狀態無效，請先重新登入。");const i=await fetch(`${f.supabaseUrl}/functions/v1/user-admin`,{method:"POST",headers:{"Content-Type":"application/json",apikey:f.supabasePublishableKey,Authorization:`Bearer ${n.session.access_token}`},body:JSON.stringify({action:e,...r})});let o=null;try{o=await i.json()}catch{o=null}if(!i.ok)throw new Error(o?.error||o?.message||`用戶管理操作失敗（HTTP ${i.status}）。`);if(!o?.ok)throw new Error(o?.error||"用戶管理操作失敗。");return o}function U(){if(!c.users.length){s.usersList.innerHTML=`
    ...
    `}).join(""),s.usersList.querySelectorAll("[data-save-user]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.dataset.saveUser,a=s.usersList.querySelector(`[data-user-group="${r}"]`)?.value||"";if(!a){t("請先選擇群組。","danger");return}e.disabled=!0;try{await l("updateGroup",{userId:r,group:a}),await d(),t("用戶權限已更新。","success")}catch(i){t(u(i),"danger")}finally{e.disabled=!1}})}),s.usersList.querySelectorAll("[data-delete-user]").forEach(e=>{e.addEventListener("click",async()=>{const r=e.dataset.deleteUser,n=c.users.find(a=>a.id===r);if(n&&window.confirm(`確定刪除用戶 ${n.email||n.id}？此操作不可還原。`)){e.disabled=!0;try{await l("deleteUser",{userId:r}),await d(),t("用戶已刪除。","success")}catch(a){t(u(a),"danger")}finally{e.disabled=!1}}})})}async function d(){const e=await l("listUsers");c.users=e.users||[],U()}async function y(e){e.preventDefault();const r=s.newUserEmail.value.trim(),n=s.newUserPassword.value,a=s.newUserGroup.value;if(!r||!n||!a){t("請完整輸入 Email、Password 和群組。","danger");return}s.createUserBtn.disabled=!0;try{await l("createUser",{email:r,password:n,group:a}),s.newUserEmail.value="",s.newUserPassword.value="",s.newUserGroup.value="staff",await d(),t("新用戶已建立。","success")}catch(i){t(u(i),"danger")}finally{s.createUserBtn.disabled=!1}}function h(){s.refreshUsersBtn.addEventListener("click",async()=>{s.refreshUsersBtn.disabled=!0;try{await d(),t("已重新載入用戶資料。","success")}catch(e){t(u(e),"danger")}finally{s.refreshUsersBtn.disabled=!1}}),s.createUserForm.addEventListener("submit",y)}async function g(){const e=await m(["superadmin","supreadmin"],"../index.html");w(e),t("正在載入用戶資料...",""),h();try{await d(),t("可管理所有用戶的群組、新增與刪除。","success")}catch(r){t(u(r),"danger")}}g();
